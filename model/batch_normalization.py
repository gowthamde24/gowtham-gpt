...
def batch_norm(x: List[List[float]], gamma: List[float], beta: List[float],
               running_mean: List[float], running_var: List[float],
               momentum: float, eps: float, training: bool) -> Tuple[List[List[float]], List[float], List[float]]:
    """Batch-normalizes a 2D batch, updating running statistics when training and using them at inference."""
    x = np.array(x)
