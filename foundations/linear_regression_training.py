...
class LinearRegressionTrainer:
    """Trains a linear regression model with plain batch gradient descent."""

    learning_rate = 0.01

    def get_derivative(self, model_prediction: NDArray[np.float64], ground_truth: NDArray[np.float64], N: int, X: NDArray[np.float64], desired_weight: int) -> float:
        """Partial derivative of the MSE loss with respect to a single weight."""
    ...
    def train_model(
        ...
        """Fits weights via batch gradient descent for a fixed number of iterations."""
